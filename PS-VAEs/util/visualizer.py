...
class Visualizer():
    ...
    table td {width: %dpx; height: %dpx; padding: 4px; outline: 4px solid black}
        ...
        if self.use_html: # save images to a html file
            for label, image_numpy in visuals.items():
                ...
                if "rec_A" in label:
                    util.save_image(image_numpy, img_path, rec_A)
                elif "rec_B" in label:
                    util.save_image(image_numpy, img_path, rec_B)
                else:
    ...
    def print_current_errors(self, epoch, i, errors, t, pre):
        message = '(epoch: %d, iters: %d, time: %.3f, acc: %.3f)\n ' % (epoch, i, t, pre)
        i = 0
